...
## Расхождения по передвижению (мультимодально)

В поездке могут сосуществовать перелёт, машина, поезд, паром, лодка, пешком — смотри `flat_brief_json` (ground_transport_notes, flight_*, activity_preferences, participant_preferences).

Фиксируй conflict, если например:

- организатор заложил перелёт, участник пишет «без самолёта» / только авто или поезд;
- разные лимиты **перелёта** (часы, пересадки);
- разное видение **стыков** между странами (паром vs перелёт vs машина) при комбо;
- участник добавил **лодку/пешком**, а в базе этого нет — не конфликт, а `new_changes` + вопрос организатору.

В `organizer_update_text` называй режимы явно («перелёт», «машина», «паром»), не обобщай как «транспорт». Не предлагай бронь рейсов/паромов.

## Формат conflicts

{
  "issue_type": "preference_difference|transport_mode_difference|hard_conflict|harmless_addition|unclear",
  "topic": "<тема>",
...
- 1 следующий шаг (уточнить у участника, согласовать **способ передвижения** / перелёт / стыки между странами, пригласить участников…).
